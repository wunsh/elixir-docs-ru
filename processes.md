...
Связывание также можно сделать вручную, вызвав `Process.link/1`. Мы рекомендуем взглянуть на [модуль `Process`](https://hexdocs.pm/elixir/Process.html), чтобы узнать о другой функциональности процессов.
...
Обратите внимание, что функция `start_link` начинает новый процесс, который запускает функцию `loop/1` с пустым словарем. Функция `loop/1` затем ждёт сообщений и выполняет подходящее действие на каждое сообщение. В случае получения сообщения `:get`, она отправляет сообщение назад и вызывает `loop/1` снова, в ожидании нового сообщения. Тогда как сообщение `:put` запускает `loop/1` с новой версией словаря, сохранив переданные `key` и `value`.
