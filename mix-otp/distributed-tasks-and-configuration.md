...
Если у вас есть другой компьютер в той же сети с установленными Эрлангом и Эликсиром, вы можете запустить на нём вторую сессию IEx. Если нет, запустите её в другом терминале. В обоих случаях дайте ей короткое имя `bar`:
...
Варианты выше имеют свои особенности. `:rpc` и использование GenServer сериализуют запрос к одному серверу, тогда как задачи будут запущены асинхронно на удалённом узле, сериализация будет произведена только на уровне порождения супервизором.
...
Распределённые задачи - ровно то же самое, что контролируемые супервизором задачи. Единственная разница в том, что мы передаём имя узла супервизору при порождении задачи. Откройте `lib/kv/supervisor.ex` из приложения `:kv`. Давайте добавим супервизор задач как последнего потомка в дереве:
