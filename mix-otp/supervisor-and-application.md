...
После старта супервизор пройдёт по списку потомков и выполнит функнцию `child_spec/1` для каждого модуля. Мы слышали о функции `child_spec/1` в главе "Агенты", когда вызывали `start_supervised(KV.Bucket)` без указания модуля.
...
Когда мы запустили супервизор, реестр автоматически запускался с заданным именем, что позволило нам создавать сегменты без необходимости ручного запуска.
...
Было бы очень неудобно обновлять этот файл вручную каждый раз, когда мы добавляем новый модуль в наше приложение. Поэтому Mix генерирует и поддерживает его актуальным за нас.
...
Ради эксперимента попробуйте переделать `KV.Registry.create/2` на использованием `GenServer.call/3` и тут же отключите обратный вызов приложения. Запустите код выше в консоли снова и вы увидите, что шаг с созданием тут же будет падать.
