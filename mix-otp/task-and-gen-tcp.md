...
Теперь сервер является частью дерева супервизора и должен запуститься автоматически при запуске приложения. Наберите `mix run --no-halt` в консоли и снова используйте клиент `telnet`, чтобы убедиться, что всё работает:
