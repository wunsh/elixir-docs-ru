...
В этой главе мы будем создавать модуль с именем `KV.Bucket`. Этот модуль будет отвечать за хранение основных значений таким образом, что позволяет им быть прочитанными и измененными другими процессами.

Если вы пропустили Getting Started guide или читали его давно, обязательно перечитайте главу [Processes]. Мы будем использовать ее в качестве отправной точки.

## проблема состояния

Эликсир это иммутабельный(immutable)  язык, в котором нет ничего общего по умолчанию. Если мы хотим сохранять состояние, создавать buckets, записывать и читать значения из нескольких мест, у нас есть два основных варианта в Эликсир:

* Processes - процессы
* [ETS (Erlang Term Storage)](http://www.erlang.org/doc/man/ets.html) - хранилище термов Эрланга

Мы уже говорили о процессах, а <abbr title="Erlang Term Storage">ЕТС</abbr> мы будем изучать далее в этом руководстве. Когда дело доходит до процессов, мы редко используем собственные реализации, вместо этого мы пользуемся абстракциями существующими в Эликсир и <abbr title="Open Telecom Platform">OTP</abbr>:

* [Agent](https://hexdocs.pm/elixir/Agent.html) - простая обертка вокруг состояния.
* [GenServer](https://hexdocs.pm/elixir/GenServer.html) - “Универсальный сервер”("Generic server") (процессы), который инкапсулирует состояние, обеспечивает синхронные и асинхронные вызововы, поддерживает горячую замену кода, и многое другое.
...
Давайте реализуем наш `KV.Bucket` используя агентов. Прежде чем приступить к выполнению, давайте сначала напишем несколько тестов. Сначала создадим файл в `test/kv/bucket_test.exs` (вспомните `.exs` расширение) со следующим содержимым:
...
Наш первый тест запускает новый `KV.Bucket` и выполняет `get/2` и `put/3` операции в нем, утверждая, результат. Нам не надо явно остонавливать агента, потому что он связан с процессом тестирования и агент выключяется автоматически после окончания теста. Это всегда будет работать, если процессу дают имя.
...
Для того, чтобы устранить сбой теста, давайте создадим файл `lib/kv/bucket.ex`.  попробуйте написать реализацию модуля `KV.Bucket` самостоятельно, используя агентов, прежде чем заглядывать в код представленный ниже.
...
Мы используем карту(map), чтобы сохранить наши ключи и значения. Об операторе Захвата `&`, вы можете почитать в [введении в руководство](/getting-started/modules-and-functions.html#function-capturing).
...
## Настройка тестов ответы(callbacks) ExUnit

Прежде чем двигаться дальше и добавлять новую функциональность для `KV.Bucket`, давайте поговорим об обратных вызовах ExUnit. Как вы можете ожидать, все тесты `KV.Bucket` затребуют корзину, чтобы быть запущенными во время установки и остановится после тестирования. К счастью, ExUnit поддерживает обратные вызовы, которые позволяют нам пропускать такие повторяющиеся задачи.

Давайте перепишем тест кейс используя обратные вызовы:
...
```
Сначала мы определили настройку обратного вызова с помощью макроса `setup/1`. В `setup/1` обратный вызов выполняется перед каждым тестом, в том же процессе в качестве самого теста.

Обратите внимание, что нам нужен механизм для передачи `bucket` pid из вызова теста. Мы делаем это с помощью тестового контекста. Когда мы возвращаем `{:ok, bucket: bucket}` из функции обратного вызова, ExUnit объединит второй элемент кортежа (список) в контекст теста. В контексте теста это карта, которую затем мы можем соответствовать в определении теста, обеспечивая доступ к этим значениям внутри блока:
...
```
Вы можете прочитать больше о ExUnit случаях в документации модуля [ExUnit.Case](https://hexdocs.pm/ex_unit/ExUnit.Case.html) и об обратных вызовах в [ExUnit.Callbacks документации](https://hexdocs.pm/ex_unit/ExUnit.Callbacks.html).

## Другие действия агента

Получать значение и обновлять состояние агента позволяет один вызов функции `Agent.get_and_update/2`. Давайте реализуем `KV.Bucket.delete/2` функцию, которая удаляет ключ из корзины, возвращая его текущее значение:
...
```
Теперь это ваша очередь, чтобы написать тест на функциональность выше! Кроме того, обязательно изучите документацию к модулю [`Agent`](https://hexdocs.pm/elixir/Agent.html), чтобы узнать больше о них.
