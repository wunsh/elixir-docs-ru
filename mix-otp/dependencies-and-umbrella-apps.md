...
Обычно стабильные релизы загружены в Hex. Если вы хотите добавить внешнюю зависимость, которая ещё находится в разработке, Mix позволяет указать git зависимость:
...
Например, если вы храните проект `kv` в git репозитории, вам нужно указать его в списке зависимостей, чтобы начать его использовать:
...
Давайте создадим новый проект Mix. Мы оригинально назовём его `kv_umbrella`, и этот проект будет включать в себя уже созданное приложение `kv` и новое, `kv_server`. Структура директорий должна выглядеть следующим образом:
...
Исходя из выведенной информации, мы можем увидеть, что сгенерировано намного меньше файлов. Сгенерированный `mix.exs` также отличается от стандартного. Давайте взглянем на него (комментарии удалены):
...
В этой главе мы узнали больше о зависимостях Mix и о зонтичных проектах. Тогда как мы можем запустить `kv` без сервера, наш `kv_server` прямо зависит от `kv`. Разделив их на изолированные приложения, мы получаем больший контроль над их разработкой и тестированием.
